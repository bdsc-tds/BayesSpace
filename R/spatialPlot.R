...
clusterPlot <- function(sce, label = "spatial.cluster",
  ...
  vertices <- .make_vertices(
    sce, label, platform, is.enhanced,
    nsubspots.per.edge
  )
...
#'
#' @return Returns a list of ggplot objects, named either \code{spot},
#'   \code{subspot}, or both.
#'
#' @family spatial plotting functions
#'
#' @importFrom assertthat assert_that
#' @importFrom SingleCellExperiment reducedDimNames SingleCellExperiment
...
.make_hex_spots <- function(cdata, fill, coord.multiplier = list(x = 1, y = 1)) {
  ...
  vertex_offsets <- data.frame(
    x.offset = c(0, r, r, 0, -r, -r),
