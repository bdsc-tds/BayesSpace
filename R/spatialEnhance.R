...
spatialEnhance <- function(sce, q, platform = c("Visium", "ST"),
                           ...
                           jitter_scale = 5, jitter_prior = 0.3, adapt.before = burn.in,
  ...
  deconv <- iterate_deconv(
    ...
    verbose = verbose, jitter_scale = jitter_scale, adapt_before = adapt.before,
    c = inputs$c, mu0 = mu0, lambda0 = lambda0, alpha = alpha, beta = beta,
    thread_num = cores
  )
