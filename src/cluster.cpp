...
iterate_deconv(
    ...
    int thin, int n, int n0, int d, const int d_subspot, double gamma, int q,
    const arma::uvec &init, int subspots, bool verbose, double jitter_scale,
    int adapt_before, double c, const NumericVector &mu0,
    const arma::mat &lambda0, double alpha, double beta, int thread_num = 1
) {
  ...
  umat df_sim_z(nrep / thin + 1, n, fill::zeros);
  mat df_sim_mu(nrep / thin + 1, d_total * q, fill::zeros);
  List df_sim_lambda(nrep / thin + 1);
  ...
  const uvec d_vector(d) = linspace<uvec>(0, d - 1, d);
  mat error(n, d);
...
#pragma omp single
      {
        ...
        if ((i + 1) % thin == 0) {
          df_sim_mu.row((i + 1) / thin) = vectorise(mu_i, 1);
          df_sim_lambda[(i + 1) / thin] = lambda_i;
          df_sim_Y[(i + 1) / thin]      = Y.cols(0, d - 1);
          df_sim_w.row((i + 1) / thin)  = w.t();
          df_sim_z.row((i + 1) / thin)  = z.t();
        }
